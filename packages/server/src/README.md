...
│   ├── contractValidation.ts # express-openapi-validator mount + spec resolution + error mapping
...
| File                         | Responsibility                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `index.ts`                   | Entry point: loads `packages/server/.env` via `import "dotenv/config"` (first import — real env vars always win; see package README), bootstraps the agent graph (`initAgentGraph()` — creates `~/.jarvis` + the checkpoint store), builds the Express app, hands it to the listener seam, and attaches the WS chat server. A thin composition root — no transport code here                                                                                         |
| `app.ts`                     | `createApp(store, appConfig)` factory → Express app: `GET /health` check, `express.json()`, the REST contract-validation gate (see `http/contractValidation.ts`), the `/api` router, a JSON-parser error handler, and — when `appConfig.portalDir` points at a built `index.html` — a strict-CSP `express.static` mount + SPA fallback (`index.html` replayed for non-`/api` GET/HEAD). Kept as a factory so tests can mount it via supertest without binding a port |
...
| `http/contractValidation.ts` | Runtime REST contract gate: resolves the OpenAPI spec from `@lukestanbery/jarvis-contracts` (Redocly bundle, else source YAML), mounts `express-openapi-validator` ahead of `/api` + `/health` — request shapes always validated, response shapes only under `JARVIS_API_CONTRACT=verify`, `validateSecurity: false` so auth stays in `middleware.ts` — and maps validator errors onto `{ error }` (400 client; server-side violations logged + generic 500)         |
